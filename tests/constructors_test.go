...
func TestNewCapturer(t *testing.T) {
	c := capture.NewCapturer(llm.NewAnthropicClient("fake-api-key"), "claude-haiku-4-5-20251001", slog.Default())
	assert.NotNil(t, c)
}

// TestNewSectionSummarizer verifies the constructor returns a non-nil summarizer.
func TestNewSectionSummarizer(t *testing.T) {
	s := indexer.NewSectionSummarizer(llm.NewAnthropicClient("fake-api-key"), "claude-haiku-4-5-20251001", slog.Default())
	assert.NotNil(t, s)
}
