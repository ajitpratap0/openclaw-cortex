...
description: "REST API reference for integrating OpenClaw Cortex with external tools and LLM pipelines"
order: 5
...
The listen address defaults to `:8080`. Configure it via environment variables:
...
Store a memory. Embeds the content and upserts it to Memgraph.
