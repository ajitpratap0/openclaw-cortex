...
func (s *MemgraphStore) InvalidateMemory(ctx context.Context, id string, invalidAt time.Time) error {
	session := s.driver.NewSession(ctx, neo4j.SessionConfig{AccessMode: neo4j.AccessModeWrite})
	defer func() { _ = session.Close(ctx) }()

	_, err := session.Run(ctx,
		`MATCH (m:Memory {uuid: $id}) SET m.valid_to = $validTo`,
		map[string]any{"id": id, "validTo": invalidAt.Format(time.RFC3339Nano)},
	)
	return err
}
...
func (s *MemgraphStore) GetHistory(ctx context.Context, id string) ([]models.Memory, error) {
	...
	// Walk the supersedes chain
	result, err := session.Run(ctx,
		`MATCH (m:Memory {uuid: $id})
		 OPTIONAL MATCH path = (m)-[:SUPERSEDES*0..10]->(old:Memory)
		 UNWIND nodes(path) AS n
		 RETURN DISTINCT n.uuid AS uuid, n.content AS content, n.created_at AS created_at, n.valid_to AS valid_to
